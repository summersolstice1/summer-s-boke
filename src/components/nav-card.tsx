...
export default function NavCard() {
	...
	const selectedIndex = hoveredIndex ?? activeIndex ?? 0
				...
				<Card
					...
					<Link className='flex items-center gap-3' href='/'>
						<Image
							...
							width={40}
							height={40}
							style={{ boxShadow: '0 12px 24px -12px var(--planet-shadow)' }}
							className='rounded-full border border-white/50'
						/>
						{form === 'full' && <span className='font-averia mt-1 text-2xl leading-none font-medium'>{siteContent.meta.title}</span>}
						{form === 'full' && <span className='text-brand mt-2 text-xs font-medium'>Planet</span>}
					</Link>
							...
							<div
								...
								<motion.div
									...
									animate={
										form === 'icons'
											? {
													left: selectedIndex * (itemHeight + 24) - extraSize,
													top: -extraSize,
									...
									style={{
										background:
											'linear-gradient(135deg, color-mix(in srgb, var(--color-brand) 18%, transparent), color-mix(in srgb, var(--color-brand-secondary) 16%, var(--color-card)))',
										borderColor: 'color-mix(in srgb, var(--color-border) 70%, transparent)',
										boxShadow: 'inset 0 1px 0 rgba(255,255,255,.35), 0 12px 28px -24px var(--planet-shadow)'
									}}
								/>
