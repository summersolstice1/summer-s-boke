...
interface RandomLayoutProps {
	...
	onEditDescription?: (picture: Picture) => void
}
...
const FloatingImage = ({
	...
	const [dragOffset, setDragOffset] = useState(() => loadSavedOffset(offsetKey))
			...
			<motion.div
				...
				onDragEnd={(_, info) => {
					if (!isZoomed) {
						...
						saveOffset(offsetKey, newOffset)
					}
				}}
				initial={{
				...
				{isEditMode && !isZoomed && (
					<>
						<div className='absolute -top-3 -right-3 flex gap-1.5' style={{ zIndex: 1 }}>
							<motion.button
								...
								className='rounded-full bg-white/90 p-1.5 text-blue-600 shadow-lg backdrop-blur transition-all hover:scale-105 hover:bg-white'>
							...
							<motion.button
								...
								className='rounded-full bg-red-500 p-1.5 text-white shadow-lg transition-all hover:scale-105 hover:bg-red-600'>
								<Trash2 className='h-3.5 w-3.5' />
							</motion.button>
						</div>
						<div className='absolute right-2 bottom-2 left-2 rounded-lg bg-black/55 px-2 py-1.5 text-xs leading-relaxed text-white opacity-0 backdrop-blur transition-opacity group-hover:opacity-100'>
							{description || '暂无备注，点击铅笔添加'}
						</div>
					</>
				)}
			</motion.div>
...
export const RandomLayout = ({ pictures, isEditMode = false, onDeleteSingle, onDeleteGroup, onEditDescription }: RandomLayoutProps) => {
	useCenterInit()
	...
	return (
		<>
			{urls.map((item, index) => {
				...
				const offsetKey = `picture-${item.pictureId}-${item.imageIndex}`
				const uniqueId = offsetKey
				...
				return (
					<FloatingImage
						...
						onEditDescription={picture ? () => onEditDescription?.(picture) : undefined}
					/>
				)
			})}
		</>
