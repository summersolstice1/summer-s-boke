...
export default function BlogPage() {
	...
	useEffect(() => {
		const handleKeyDown = (e: KeyboardEvent) => {
			if (!hideEditButton && !editMode && (e.ctrlKey || e.metaKey) && e.key === ',') {
				e.preventDefault()
				toggleEditMode()
			}
		}
	...
	}, [editMode, hideEditButton, toggleEditMode])
			...
			<motion.div
				...
				{editMode ? (
					<>
						...
						<motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleSave} disabled={saving} className='brand-btn px-6'>
							{saving ? '保存中...' : '保存'}
						</motion.button>
					</>
				) : (
