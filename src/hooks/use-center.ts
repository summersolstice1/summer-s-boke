...
export const useCenterStore = create<CenterState>(set => ({
	...
	recalc: () => {
		...
		set(state => {
			if (state.x === c.x && state.y === c.y && state.width === c.width && state.height === c.height) {
				return state
			}

			return { x: c.x, y: c.y, width: c.width, height: c.height, centerX: c.centerX, centerY: c.centerY }
		})
	}
}))
